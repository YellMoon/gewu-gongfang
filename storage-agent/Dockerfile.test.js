...
assert.match(dockerfile, /^RUN apt-get update && apt-get install -y --no-install-recommends python3 && rm -rf \/var\/lib\/apt\/lists\/\*$/m, 'the Word parser requires Python without leaving package-manager cache in the image');
