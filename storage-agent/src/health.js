...
async function runStorageAgentHealthCheck({ config, version, randomId = () => crypto.randomUUID(), now = () => new Date() } = {}) {
  ...
  const healthDirectory = assertInsideRoot(path.join(root, '.gewu-storage-agent', 'health'), root);
  const probePath = assertInsideRoot(path.join(healthDirectory, `${probeId}.probe`), root);
