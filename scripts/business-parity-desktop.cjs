...
async function main() {
  ...
  save('qa-inventory', {checks:[
    ...
    '启动尺寸及较小窗口截图，无遮挡和裁切'], scope:config.studentDeleteHistory?'student-delete-history-only':config.studentBalanceFixture?'student-balance-only':config.confirmedDeleteUndoOnly?'confirmed-delete-undo-only':config.courseAddressOnly?'course-address-only':config.courseConfirmationOnly?'course-confirmation-only':'business-parity', fullSignoff:false});
  ...
  try {
    ...
    if(config.resourceMaintenanceOnly){
      save('qa-inventory',{scope:'resource-maintenance-only',checks:['原机构与地址新增编辑删除','重连不发送、用户明确确认','机构学生选入原课程并排课','地址改名不重写课次','删除后历史保留、重开与实际云库核验'],fullSignoff:false});
      const result=await require('./business-parity-resource-maintenance.cjs')({page,out,save,releaseNavigation,teacherId:config.login.teacherId});
      save('desktop-receipt',{scope:'resource-maintenance-only',...result,sourceDesktop:true,installed:false,productionWrite:false,uiVerified:false,businessFlowComplete:false});
      return;
    }
    ...
    if(config.courseActiveOnly){
      ...
      save('desktop-receipt',{scope:'course-active-only',result,sourceDesktop:true,installed:false,productionWrite:false,uiVerified:false,businessFlowComplete:false});
      return;
    }
