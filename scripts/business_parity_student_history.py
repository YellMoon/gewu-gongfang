...
def seed_student_history(db, target, parents_deleted=True):
    ...
    db.run("BEGIN; SET LOCAL ROLE vnext_pg17_business_owner; DO $$ BEGIN IF current_database()<>" + q(target) +
           ...
           "INSERT INTO business.courses(id,tenant_id,name,display_name,course_type,legacy_source_type,price_tuition,price_teacher,billing_unit,teacher_fee_mode,teacher_id,legacy_active,legacy_deleted,created_at,updated_at) SELECT id,tenant_id,'历史课程核验','历史课程核验',1,1,180,120,1,1,created_by_teacher_id,true," + archived + ",now(),now() FROM business.students WHERE id=" + student + "; " +
           ...
           "INSERT INTO business.schedules(id,tenant_id,course_id,start_at,end_at,status,calculated_tuition,calculated_teacher_fee,billing_unit,teacher_fee_mode,teacher_id,legacy_deleted,created_at,updated_at) SELECT id,tenant_id,id,'2026-09-08T01:00:00Z'::timestamptz,'2026-09-08T02:30:00Z'::timestamptz,1,270,180,1,1,created_by_teacher_id," + archived + ",now(),now() FROM business.students WHERE id=" + student + "; " +
