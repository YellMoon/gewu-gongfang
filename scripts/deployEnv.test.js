...
for (const name of ['APP_ENV', 'SCHEDULE_ENV', 'PORT', 'DEPLOY_HOST', 'DEPLOY_PASSWORD', 'DEPLOY_KEY_PATH', 'BACKEND_JWT_SECRET']) {
  delete cleanEnv[name];
}
cleanEnv.DOTENV_CONFIG_PATH = envFixturePath;
const deployProbe = spawnSync('python', [
  '-c',
  'import scripts.deploy as d; print(d.APP_ENV); print(d.HOST); print(d.APP_PORT); print("GEWU_HOST_BASE_URL=http://127.0.0.1:3002" in d.remote_env_prefix()); print("s3cr3t" not in d.redact_command(d.remote_env_prefix()))',
...
assert.deepStrictEqual(
  ...
  ['prod', 'deploy-env-test-host', '3002', 'True', 'True'],
