...
def state_sql(upgrade):
    ...
    statement = (
        ...
        "'startFixed',(SELECT position('s.status = ''active''' in pg_get_functiondef(p.oid))>0 AND position('s.expires_at > now_at' in pg_get_functiondef(p.oid))>0 AND position('FOR SHARE OF s' in pg_get_functiondef(p.oid))>0 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='vnext_control_plane' AND p.proname='vnext_start_desktop_session_challenge'),"
        "'exchangeFixed',(SELECT position('source_session vnext_control_plane.vnext_sessions%ROWTYPE' in pg_get_functiondef(p.oid))>0 AND position('source_session.status <> ''active''' in pg_get_functiondef(p.oid))>0 AND position('FOR UPDATE' in pg_get_functiondef(p.oid))>0 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='vnext_control_plane' AND p.proname='vnext_exchange_desktop_session_challenge'),"
