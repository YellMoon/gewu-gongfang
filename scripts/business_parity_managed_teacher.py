...
def read_managed_teacher(db, ui, creator):
    ...
    value = json.loads(db.run(f"""SELECT jsonb_build_object(
      'teacher',(SELECT jsonb_build_object('id',t.id,'name',t.name,'creator',t.created_by_teacher_id,'claimed',t.account_claimed,'rate',t.hourly_rate,'deleted',t.legacy_deleted) FROM business.teachers t WHERE t.id='{teacher}'),
    ...
    expected = {
        'teacher': {'id': teacher, 'name': '周启明', 'creator': creator, 'claimed': False, 'rate': 120, 'deleted': True},
    ...
    if not all(ui.get(key) is True for key in ('teacherDeleted', 'historyPreserved', 'reloaded', 'reconnectDidNotSubmit')):
        raise RuntimeError('MANAGED_TEACHER_UI_DELETION_EVIDENCE_REQUIRED')
    return {'actual': value, 'expected': expected, 'verified': True, 'teacherDeletionVerified': True}
