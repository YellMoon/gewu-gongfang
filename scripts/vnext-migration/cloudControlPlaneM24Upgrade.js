...
function buildCloudControlPlaneM24UpgradeSql() {
  const prefix = MIGRATIONS.slice(0, 23); const migration = MIGRATIONS[23];
  if (prefix.length !== 23 || !migration || migration.semanticVersion !== 24
  ...
  const lines = [
    ...
    `DO $$ BEGIN IF (SELECT count(*) FROM vnext_control_plane.vnext_schema_migrations) <> 23 OR EXISTS (SELECT 1 FROM (VALUES ${expectedRows}) AS expected(migration_id,semantic_version,manifest_sha256) LEFT JOIN vnext_control_plane.vnext_schema_migrations actual USING (migration_id,semantic_version,manifest_sha256) WHERE actual.migration_id IS NULL) THEN RAISE EXCEPTION 'VNEXT_CLOUD_CONTROL_PLANE_M23_PREFIX_INVALID' USING ERRCODE='P0001'; END IF; END $$;`,
    'GRANT vnext_pg17_owner TO gewu_app;', 'SET LOCAL ROLE vnext_pg17_owner;', migration.sql,
    `INSERT INTO vnext_control_plane.vnext_schema_migrations (migration_id, semantic_version, manifest_sha256, applied_at, applied_by) VALUES (${sqlLiteral(migration.migrationId)}, ${migration.semanticVersion}, ${sqlLiteral(migration.manifestSha256)}, transaction_timestamp(), 'gewu-cloud-control-m24-upgrade');`,
...
module.exports = Object.freeze({ buildCloudControlPlaneM24UpgradeSql });
