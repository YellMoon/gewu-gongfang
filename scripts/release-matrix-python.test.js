...
try {
  ...
  const probe = spawnSync('python', ['-c', [
    ...
    "manifest = deploy.require_release_manifest('backend')",
    "deploy.record_release_receipt('backend', 'unit health receipt')",
  ...
  assert.strictEqual(probe.status, 0, probe.stderr || probe.stdout || 'backend release manifest probe must pass');
  assert.strictEqual(probe.stdout.trim(), rootPackage.version, 'backend receipt gate should use the exact root version');
  const recorded = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  assert.strictEqual(recorded.targets.backend.status, 'verified');
  assert.strictEqual(recorded.targets.backend.receipt.version, rootPackage.version);
  assert.throws(
    () => matrix.recordReceipt(recorded, { target: 'backend', version: rootPackage.version, evidence: 'duplicate' }),
