...
const V_NEXT_CONTROL_PLANE_REFERENCE_TABLES = Object.freeze([
  ...
  'vNext_authorization_audit_events', 'vNext_capability_catalog',
...
const version = name => `${name} INTEGER NOT NULL CHECK(${name}>=1)`;

const STATEMENTS = Object.freeze([
  `CREATE TABLE IF NOT EXISTS vNext_schema_meta (${id('schema_key')} CHECK(schema_key='control-plane-reference'), schema_version INTEGER NOT NULL CHECK(schema_version=1), ${time('applied_at')})`,
  ...
  `CREATE TABLE IF NOT EXISTS vNext_authorization_audit_events (${id('event_id')}, authority_id TEXT NOT NULL ${NONEMPTY.replace('%s', 'authority_id')}, actor_key TEXT NOT NULL ${NONEMPTY.replace('%s', 'actor_key')}, actor_account_id TEXT, action TEXT NOT NULL ${NONEMPTY.replace('%s', 'action')}, reason_code TEXT NOT NULL ${NONEMPTY.replace('%s', 'reason_code')}, idempotency_key TEXT NOT NULL ${NONEMPTY.replace('%s', 'idempotency_key')}, outcome TEXT NOT NULL CHECK(outcome IN ('accepted','rejected','revoked')), auth_version INTEGER CHECK(auth_version IS NULL OR auth_version>=1), access_version INTEGER CHECK(access_version IS NULL OR access_version>=1), revocation_version INTEGER CHECK(revocation_version IS NULL OR revocation_version>=1), ${time('created_at')}, UNIQUE(authority_id,actor_key,idempotency_key), FOREIGN KEY(authority_id) REFERENCES vNext_authorities(authority_id), FOREIGN KEY(actor_account_id,authority_id) REFERENCES vNext_accounts(account_id,authority_id))`,
...
const REQUIRED_COLUMNS = Object.freeze({
  ...
  vNext_authorization_audit_events: ['event_id','authority_id','actor_key','actor_account_id','action','reason_code','idempotency_key','outcome','auth_version','access_version','revocation_version','created_at'],
});
...
function assertSchema(db) {
  ...
  const indexes = db.prepare("SELECT name,sql FROM sqlite_master WHERE type='index' AND name LIKE 'vNext_%' AND sql IS NOT NULL").all();
  ...
  const triggers = db.prepare("SELECT name,sql FROM sqlite_master WHERE type='trigger' AND name LIKE 'vNext_authorization_audit_events_%'").all();
...
function bootstrapVNextControlPlaneReference(db, { now = () => new Date().toISOString(), testHooks = {} } = {}) {
  ...
  const apply = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO vNext_schema_meta(schema_key,schema_version,applied_at) VALUES('control-plane-reference',1,?) ON CONFLICT(schema_key) DO NOTHING`).run(String(now()));
    const meta = db.prepare("SELECT schema_version FROM vNext_schema_meta WHERE schema_key='control-plane-reference'").get();
    if (!meta || Number(meta.schema_version) !== 1) throw kernelError('VNEXT_REFERENCE_SCHEMA_DRIFT');
    assertSchema(db);
  });
  apply();
  return Object.freeze({ schemaVersion: 1, tables: V_NEXT_CONTROL_PLANE_REFERENCE_TABLES });
}
