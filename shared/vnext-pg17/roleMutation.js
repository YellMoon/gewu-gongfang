  ...
  async function execute(assertion, input) {
    ...
    return withVNextPg17SyntheticQuery(settings.handle, 'fixture-provisioner', async facade => {
      try {
        ...
        if (grant.role === 'super_admin') { const admins = await facade.query("SELECT grant_id FROM vnext_control_plane.vnext_role_grants WHERE authority_id=$1 AND role='super_admin' AND status='active' FOR UPDATE", [authorityId]); if (admins.rows.length === 1) { const value = await record({ outcome: 'rejected', code: 'LAST_SUPER_ADMIN_REVOKE_FORBIDDEN', result: { code: 'LAST_SUPER_ADMIN_REVOKE_FORBIDDEN', status: 'rejected' }, targetKind: 'role_grant', targetId: grant.grant_id }); await facade.query('COMMIT'); return value; } }
