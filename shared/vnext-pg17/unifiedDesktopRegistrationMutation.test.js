...
async function runUnifiedDesktopRegistrationMutationCases(runtime) {
  ...
  try {
    ...
    await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query(
      "INSERT INTO vnext_control_plane.vnext_accounts(account_id,authority_id,status,auth_version,access_version,revocation_version,row_version,created_at,updated_at) VALUES('account-collision','authority-1','active',1,1,1,1,$1,$1)", [AT],
    ));
    await issueVNextPg17OnlineIdentityAssertion(activeRuntime, handle, { ...assertion, assertionId: 'assertion-outbox-collision', accountId: 'account-collision', deviceId: 'device-collision', installationId: 'installation-collision', installationPublicKey: 'public-key-collision', keyFingerprint: hash('8'), nonceSha256: hash('c') });
    const beforeOutboxCollision = await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query("SELECT (SELECT count(*)::text FROM vnext_control_plane.vnext_trusted_devices) AS devices, (SELECT count(*)::text FROM vnext_control_plane.vnext_device_installations) AS installations, (SELECT count(*)::text FROM vnext_control_plane.vnext_account_device_links) AS links, (SELECT count(*)::text FROM vnext_control_plane.vnext_sessions) AS sessions, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_command_receipts) AS receipts, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_audit_events) AS audits, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_outbox_events) AS outbox"));
    await assert.rejects(() => registerVNextPg17UnifiedDesktopOnline(activeRuntime, handle, Object.assign({ ...registration, assertionId: 'assertion-outbox-collision', idempotencyKey: 'idempotency-outbox-collision', receiptId: 'receipt-outbox-collision', auditEventId: 'audit-outbox-collision', outboxEventId: 'outbox-1', sessionId: 'session-outbox-collision', linkId: 'link-outbox-collision' }, createUnifiedDesktopRegistrationEvidence({ sessionId: 'session-outbox-collision' }))));
    const afterOutboxCollision = await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query("SELECT (SELECT count(*)::text FROM vnext_control_plane.vnext_trusted_devices) AS devices, (SELECT count(*)::text FROM vnext_control_plane.vnext_device_installations) AS installations, (SELECT count(*)::text FROM vnext_control_plane.vnext_account_device_links) AS links, (SELECT count(*)::text FROM vnext_control_plane.vnext_sessions) AS sessions, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_command_receipts) AS receipts, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_audit_events) AS audits, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_outbox_events) AS outbox"));
    assert.deepStrictEqual(afterOutboxCollision.rows, beforeOutboxCollision.rows);
    for (const [suffix, nonce, duplicateField, duplicateId] of [
      ['receipt', 'd', 'receiptId', 'receipt-1'],
      ['audit', 'e', 'auditEventId', 'audit-1'],
    ]) {
      await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query(
        `INSERT INTO vnext_control_plane.vnext_accounts(account_id,authority_id,status,auth_version,access_version,revocation_version,row_version,created_at,updated_at) VALUES('account-${suffix}-collision','authority-1','active',1,1,1,1,$1,$1)`, [AT],
      ));
      await issueVNextPg17OnlineIdentityAssertion(activeRuntime, handle, { ...assertion, assertionId: `assertion-${suffix}-collision`, accountId: `account-${suffix}-collision`, deviceId: `device-${suffix}-collision`, installationId: `installation-${suffix}-collision`, installationPublicKey: `public-key-${suffix}-collision`, keyFingerprint: hash(nonce), nonceSha256: hash(nonce) });
      const before = await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query("SELECT (SELECT count(*)::text FROM vnext_control_plane.vnext_trusted_devices) AS devices, (SELECT count(*)::text FROM vnext_control_plane.vnext_device_installations) AS installations, (SELECT count(*)::text FROM vnext_control_plane.vnext_account_device_links) AS links, (SELECT count(*)::text FROM vnext_control_plane.vnext_sessions) AS sessions, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_command_receipts) AS receipts, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_audit_events) AS audits, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_outbox_events) AS outbox"));
      const candidate = Object.assign({ ...registration, assertionId: `assertion-${suffix}-collision`, idempotencyKey: `idempotency-${suffix}-collision`, receiptId: `receipt-${suffix}-collision`, auditEventId: `audit-${suffix}-collision`, outboxEventId: `outbox-${suffix}-collision`, sessionId: `session-${suffix}-collision`, linkId: `link-${suffix}-collision` }, createUnifiedDesktopRegistrationEvidence({ sessionId: `session-${suffix}-collision` }));
      candidate[duplicateField] = duplicateId;
      await assert.rejects(() => registerVNextPg17UnifiedDesktopOnline(activeRuntime, handle, candidate));
      const after = await withVNextPg17SyntheticQuery(handle, 'fixture-provisioner', facade => facade.query("SELECT (SELECT count(*)::text FROM vnext_control_plane.vnext_trusted_devices) AS devices, (SELECT count(*)::text FROM vnext_control_plane.vnext_device_installations) AS installations, (SELECT count(*)::text FROM vnext_control_plane.vnext_account_device_links) AS links, (SELECT count(*)::text FROM vnext_control_plane.vnext_sessions) AS sessions, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_command_receipts) AS receipts, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_audit_events) AS audits, (SELECT count(*)::text FROM vnext_control_plane.vnext_authorization_outbox_events) AS outbox"));
