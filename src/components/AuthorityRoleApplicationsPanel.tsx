...
function reviewErrorMessage(error: any) {
  const messages: Record<string, string> = {
    // UTF-8: Keep transport details out of user-facing messages.
    DESKTOP_SUPER_ADMIN_SESSION_REQUIRED: '当前账号没有审核权限',
    AUTHORIZATION_CONTEXT_REQUIRED: '登录已失效，请重新登录',
    DESKTOP_CLOUD_SESSION_REQUIRED: '登录已失效，请重新登录',
    DESKTOP_OFFLINE_DRAFT_SUBMISSION_FORBIDDEN: '请连接网络后再审核',
  ...
  return messages[error?.code] || '暂时无法处理申请，请稍后重试';
}

function reviewConfirmationText(application: RoleApplication) {
...
const AuthorityRoleApplicationsPanel: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setVisible(true);
      setItems([]);
      setErrorCode(error?.code || 'AUTHORITY_ROLE_PROJECTION_UNAVAILABLE');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      >
        <Alert
          ...
          showIcon
          message={errorCode ? reviewErrorMessage({ code: errorCode }) : '请核对申请人的姓名和手机号'}
              ...
              },
              {
                ...
                render: (_value, item) => <Space>
                  <Button
                    ...
                    onClick={() => Modal.confirm({ title: '确认通过申请', content: reviewConfirmationText(item), onOk: () => queueDecision(item, 'approved') })}
                  >{'\u786e\u8ba4\u901a\u8fc7'}</Button>
                  <Button
                    danger
                    loading={busyId === item.applicationId}
