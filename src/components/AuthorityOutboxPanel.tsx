...
const copy = {
  ...
  completed: '\u5df2\u5b8c\u6210',
  issue: '\u9700\u8981\u5904\u7406',
  modalTitle: '\u786e\u8ba4\u63d0\u4ea4\u5230\u6743\u5a01\u6570\u636e\u4e3b\u673a',
  commandType: '\u547d\u4ee4\u7c7b\u578b',
  preview: '\u5f71\u54cd\u9884\u89c8',
  safety: '\u5b89\u5168\u8bf4\u660e',
  safetyText: '\u786e\u8ba4\u540e\u624d\u4f1a\u53d1\u9001\uff1b\u4e3b\u673a\u5c06\u91cd\u65b0\u6821\u9a8c\u8bbe\u5907\u3001\u79df\u7ea6\u3001\u89d2\u8272\u3001\u5b57\u6bb5\u8303\u56f4\u548c\u6743\u5a01 epoch\u3002',
  confirm: '\u786e\u8ba4\u5e76\u53d1\u9001',
  keep: '\u7ee7\u7eed\u4fdd\u7559\u8349\u7a3f',
  retry: '\u91cd\u8bd5\u540c\u4e00\u547d\u4ee4',
  retained: '\u4fdd\u7559\uff0c\u7b49\u5f85\u5904\u7406',
  ...
  transport: '\u4f20\u8f93 / \u6295\u5f71',
  actions: '\u64cd\u4f5c',
  authorityMessage: '\u6240\u6709\u4e1a\u52a1\u5199\u5165\u5747\u7531\u6743\u5a01\u6570\u636e\u4e3b\u673a\u88c1\u51b3',
  conflictMessage: '\u5b58\u5728\u56de\u6267\u51b2\u7a81\uff0c\u8349\u7a3f\u5df2\u4fdd\u7559',
  authorityDescription: '\u79bb\u7ebf\u7f16\u8f91\u5148\u8fdb\u5165\u672c\u673a\u52a0\u5bc6\u547d\u4ee4\u7bb1\uff1b\u53ea\u6709\u660e\u786e\u786e\u8ba4\u540e\uff0c\u624d\u6309 LAN WebSocket\u3001\u4e91\u4e2d\u7ee7 WebSocket\u3001\u6301\u4e45\u4e2d\u7ee7\u7684\u987a\u5e8f\u53d1\u9001\u540c\u4e00\u6761\u7b7e\u540d\u547d\u4ee4\u3002',
  empty: '\u5f53\u524d\u6ca1\u6709\u7b26\u5408\u6761\u4ef6\u7684\u6743\u5a01\u547d\u4ee4',
  wsTitle: 'WebSocket \u53ea\u8d1f\u8d23\u964d\u4f4e\u5ef6\u8fdf',
  wsDescription: '\u5373\u4f7f LAN \u6216\u4e91\u4e2d\u7ee7 socket \u65ad\u5f00\uff0c\u5df2\u786e\u8ba4\u7684\u6301\u4e45\u547d\u4ee4\u4ecd\u7531\u4e3b\u673a\u72ec\u7acb worker \u8f6e\u8be2\u6062\u590d\uff1b\u4e0d\u4f1a\u9000\u56de\u539f\u59cb\u884c\u540c\u6b65\u3002',
};
...
const AuthorityOutboxPanel: React.FC<Props> = ({ compact = false, focus }) => {
  ...
  const confirmAndSubmit = (item: AuthorityOutboxItem) => {
    Modal.confirm({
      ...
      onOk: async () => {
        setBusyId(item.id);
        try {
          const result = await requireBridge().confirmAndSubmit(item.id);
  ...
  const retry = async (item: AuthorityOutboxItem) => {
    setBusyId(item.id);
    try {
      const result = await requireBridge().submit(item.id);
      if (result) {
        if (result.receipt?.status === 'rejected') {
          message.error(result.receipt?.result?.error?.code || 'AUTHORITY_COMMAND_REJECTED');
        } else {
