...
(async function main() {
  ...
  const businessOutbox = createDesktopCommandOutbox({
    ...
    createId: () => `business-draft-${++businessDraftSequence}`, now: () => '2026-08-24T00:00:00.000Z',
  });
  ...
  // Every business collection generated by browserDatabase must be recognised
  // here.  The legacy envelope and transport above deliberately throw, so this
  // is an exhaustive runtime guard against accidentally adding a business
  // draft that falls back to /api/authority/commands.
  ...
  for (const entity of cloudBusinessEntities) {
    for (const action of ['create', 'update', 'delete']) {
      const id = `${entity}-${action}-1`;
      const payload = action === 'create'
        ? { record: { id } }
        : { id, expectedVersion: '2026-08-24T00:00:00.000Z', ...(action === 'update' ? { changes: { name: 'edited' } } : {}) };
      const draft = await businessClient.appendDraft({ type: `${entity}.${action}.v1`, payload });
      const submitted = await businessClient.confirmAndSubmit(draft.id);
      assert.strictEqual(submitted.transportUsed, 'cloud-business-authority', `${entity}.${action} must use cloud business authority`);
      assert.strictEqual((await businessOutbox.get(draft.id)).status, 'completed');
    }
  }
  assert.strictEqual(businessSubmissions, 1 + (cloudBusinessEntities.length * 3),
    'each typed business draft must be submitted directly to cloud business authority');
