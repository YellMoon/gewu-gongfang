...
const sql = fs.readFileSync(path.join(__dirname, '20260823-storage-agent-least-privilege.sql'), 'utf8');

assert.match(sql, /^BEGIN;[\s\S]*COMMIT;\s*$/u);
assert.match(sql, /GRANT SELECT,UPDATE ON TABLE business\.storage_object_tasks TO gewu_cloud_schedule_reader;/u);
...
console.log('storage agent least privilege SQL checks passed');
