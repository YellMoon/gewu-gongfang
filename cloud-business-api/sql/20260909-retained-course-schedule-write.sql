-- UTF-8: original retained-lesson move/copy/resize/delete; tombstones retain ownership.
-- Forward-only correction. No selector resurrection, roster cascade or UI changes.
BEGIN;
...
BEGIN
  ...
  FOR checked_student_id IN SELECT DISTINCT x FROM unnest(p_student_ids) x ORDER BY x LOOP
    IF NOT business.vnext_teacher_student_access(p_tenant_id,checked_student_id,p_teacher_id) THEN
      -- A retained lesson carries its original enrolments even after the course
      -- leaves selectors. Do not broaden student CRUD or other course ownership.
      PERFORM 1 FROM business.students st
        ...
        WHERE st.tenant_id=p_tenant_id AND st.id=checked_student_id AND st.legacy_deleted=false
          AND p.course_id=COALESCE(p_course_id,current_course)
        FOR SHARE OF st,p;
      IF NOT FOUND THEN
        PERFORM 1 FROM business.students st
          ...
          WHERE st.tenant_id=p_tenant_id AND st.id=checked_student_id AND st.legacy_deleted=false
            AND s.course_id=COALESCE(p_course_id,current_course) AND s.legacy_deleted=false
