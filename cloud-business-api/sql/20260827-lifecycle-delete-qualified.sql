...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM business.courses AS course_record WHERE course_record.tenant_id=p_tenant_id AND course_record.teacher_id=p_teacher_id AND course_record.legacy_deleted=false) THEN
    RAISE EXCEPTION 'VNEXT_BUSINESS_TEACHER_REFERENCED' USING ERRCODE = 'P0001';
  END IF;
  UPDATE business.teachers AS target SET legacy_deleted=true,updated_at=date_trunc('milliseconds',transaction_timestamp())
   WHERE target.tenant_id=p_tenant_id AND target.id=p_teacher_id AND target.legacy_deleted=false AND target.updated_at=p_expected_updated_at
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM business.courses AS course_record WHERE course_record.tenant_id=p_tenant_id AND course_record.legacy_room_id=p_room_id AND course_record.legacy_deleted=false) THEN
    RAISE EXCEPTION 'VNEXT_BUSINESS_ROOM_REFERENCED' USING ERRCODE = 'P0001';
  END IF;
  UPDATE business.rooms AS target SET legacy_deleted=true,updated_at=date_trunc('milliseconds',transaction_timestamp())
   WHERE target.tenant_id=p_tenant_id AND target.id=p_room_id AND target.legacy_deleted=false AND target.updated_at=p_expected_updated_at
  RETURNING target.id,target.updated_at INTO id,updated_at;
  IF NOT FOUND THEN RETURN; END IF;
  RETURN NEXT;
END;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM business.course_student_pricings AS pricing WHERE pricing.tenant_id=p_tenant_id AND pricing.student_id=p_student_id)
    OR EXISTS (SELECT 1 FROM business.schedule_student_overrides AS override_record WHERE override_record.tenant_id=p_tenant_id AND override_record.student_id=p_student_id) THEN
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM business.schedules AS schedule_record WHERE schedule_record.tenant_id=p_tenant_id AND schedule_record.course_id=p_course_id)
    OR EXISTS (SELECT 1 FROM business.schedule_student_overrides AS override_record WHERE override_record.tenant_id=p_tenant_id AND override_record.schedule_id IN (SELECT schedule_record.id FROM business.schedules AS schedule_record WHERE schedule_record.tenant_id=p_tenant_id AND schedule_record.course_id=p_course_id)) THEN
