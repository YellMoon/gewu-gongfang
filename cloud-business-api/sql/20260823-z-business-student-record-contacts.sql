...
CREATE OR REPLACE FUNCTION business.vnext_update_student_record_v3(
  p_tenant_id text,
  p_student_id text,
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE c.slot NOT BETWEEN 1 AND 3
      ...
      OR (c.phone IS NULL AND c.wechat IS NULL)
...
REVOKE ALL ON FUNCTION business.vnext_update_student_record_v3(text,text,timestamptz,text,text,integer,text,text,text,text,integer,text,jsonb) FROM PUBLIC;
GRANT USAGE ON SCHEMA business TO vnext_pg17_writer;
GRANT EXECUTE ON FUNCTION business.vnext_update_student_record_v3(text,text,timestamptz,text,text,integer,text,text,text,text,integer,text,jsonb) TO vnext_pg17_writer;
