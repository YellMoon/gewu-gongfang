...
function createPaperExportMediaResolver({ questionAssetDeliveries } = {}) {
  if (!questionAssetDeliveries || typeof questionAssetDeliveries.request !== 'function' || typeof questionAssetDeliveries.download !== 'function') {
  ...
  return async input => {
    ...
    const delivery = await questionAssetDeliveries.request({ tenantId, accountId, questionId, assetKey });
