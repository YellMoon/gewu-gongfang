...
function validate(input) {
  const request = input && typeof input === 'object' && Object.hasOwn(input, 'parserSha256')
    ? exact(input, ['agentId', 'agentVersion', 'contracts', 'parserSha256'])
    : exact(input, ['agentId', 'agentVersion', 'contracts']);
  ...
  if (request.contracts?.storageAgentTransport === 2) {
    const contracts = exact(request.contracts, ['questionPaperExport', 'storageAgentTransport']);
    if (contracts.questionPaperExport !== 3 || (Object.hasOwn(request, 'parserSha256') && request.parserSha256 !== null)) throw failure();
    return { agentId: request.agentId, agentVersion: request.agentVersion, contracts, parserSha256: null };
  }
  const contracts = exact(request.contracts, ['questionPaperExport', 'storageAgentTransport', 'questionImportParserProof']);
  if (contracts.questionPaperExport !== 3 || contracts.storageAgentTransport !== 3 || contracts.questionImportParserProof !== 1
    || typeof request.parserSha256 !== 'string' || !/^[0-9a-f]{64}$/.test(request.parserSha256)) throw failure();
  return { agentId: request.agentId, agentVersion: request.agentVersion, contracts, parserSha256: request.parserSha256 };
}
...
function output(row) {
  ...
  const validated = validate({
    agentId: row.agentId, agentVersion: row.agentVersion, contracts: row.contracts, parserSha256: row.parserSha256,
  });
  return Object.freeze({
    receiptId: row.receiptId, agentId: row.agentId, agentVersion: row.agentVersion,
    contracts: { ...validated.contracts },
    ...(validated.parserSha256 === null ? {} : { parserSha256: validated.parserSha256 }),
  ...
  return Object.freeze({
    async record(input) {
      ...
      const result = await query(
        `INSERT INTO business.storage_agent_runtime_receipts (receipt_id,agent_id,agent_version,contracts,parser_sha256)
         VALUES ($1,$2,$3,$4::jsonb,$5)
         RETURNING receipt_id AS "receiptId",agent_id AS "agentId",agent_version AS "agentVersion",contracts,parser_sha256 AS "parserSha256",observed_at AS "observedAt"`,
        [receiptId, request.agentId, request.agentVersion, JSON.stringify(request.contracts), request.parserSha256],
      );
