...
function createQuestionAssetDeliveryRepository({ query, randomId = () => crypto.randomUUID(), randomToken = () => crypto.randomBytes(32).toString('base64url'), now = () => new Date(), ttlSeconds = 900, leaseSeconds = 300 } = {}) {
  ...
  return Object.freeze({
    ...
    async requestForPaperExport(input) {
      ...
      const result = await query([
        'WITH purged AS (DELETE FROM business.question_asset_deliveries WHERE expires_at<=transaction_timestamp()),',
        'asset AS (SELECT asset.id AS "assetId",asset.storage_object_id AS "objectId",asset.storage_object_version AS "objectVersion",asset.content_hash AS "expectedSha256",asset.size_bytes AS "expectedBytes",',
        "COALESCE(NULLIF(asset.file_name,''),'question-asset') AS \"fileName\",asset.mime_type AS \"mimeType\",LEAST($7::timestamptz,task.created_at+interval '1 hour') AS \"mediaExpiresAt\"",
        'FROM business.question_assets asset JOIN business.questions question ON question.tenant_id=asset.tenant_id AND question.id=asset.question_id',
        "JOIN business.paper_export_tasks task ON task.tenant_id=asset.tenant_id AND task.account_id=$2 AND task.task_id=$5 AND task.status='processing' AND task.created_at>transaction_timestamp()-interval '1 hour'",
        "WHERE asset.tenant_id=$1 AND asset.content_hash=$3 AND asset.question_id=$4 AND asset.state='verified' AND asset.deleted=false AND question.deleted=false AND task.question_snapshot_json @> jsonb_build_array(jsonb_build_object('id',$4,'assets',jsonb_build_array(jsonb_build_object('assetKey',$3)))) ORDER BY asset.created_at DESC,asset.id DESC LIMIT 1),",
        // Only a currently authorized export may extend its own temporary media.
        // Preserve NAS lease and bytes; cap the task's preparation at one hour.
        'selected AS (SELECT delivery_id FROM business.question_asset_deliveries WHERE tenant_id=$1 AND account_id=$2 AND expected_sha256=$3 AND asset_id IN (SELECT "assetId" FROM asset)',
        "AND expires_at>transaction_timestamp() AND status IN ('queued','leased','ready') ORDER BY created_at DESC LIMIT 1),",
        'existing AS (UPDATE business.question_asset_deliveries delivery SET expires_at=GREATEST(delivery.expires_at,asset."mediaExpiresAt") FROM selected,asset WHERE delivery.delivery_id=selected.delivery_id',
        'RETURNING delivery.delivery_id AS "deliveryId",delivery.status,delivery.asset_id AS "assetId",delivery.file_name AS "fileName",delivery.mime_type AS "mimeType",delivery.expires_at AS "expiresAt"),',
        'created AS (INSERT INTO business.question_asset_deliveries(delivery_id,asset_id,tenant_id,account_id,object_id,object_version,expected_sha256,expected_bytes,file_name,mime_type,status,expires_at)',
        'SELECT $6,asset."assetId",$1,$2,asset."objectId",asset."objectVersion",asset."expectedSha256",asset."expectedBytes",asset."fileName",asset."mimeType",\'queued\',asset."mediaExpiresAt" FROM asset WHERE NOT EXISTS (SELECT 1 FROM existing)',
