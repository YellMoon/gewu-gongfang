...
const query = async (text, values) => {
  ...
  if (text.includes('WITH target AS')) {
    return { rows: [{ accountId: 'account-pending', status: 'active', roles: ['teacher'] }] };
  }
...
(async () => {
  ...
  assert.deepStrictEqual(await repository.assignRole({ accountId: 'account-pending', role: 'teacher' }), { accountId: 'account-pending', status: 'active', roles: ['teacher'] });
