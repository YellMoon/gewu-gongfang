...
async function main() {
  ...
  const runtime = createStorageAgentRuntimeFromEnvironment({
    ...
    query: async (text, values) => {
      ...
      if (text.includes('storage_agent_runtime_receipts')) return { rows: [{
        ...
        contracts: { questionPaperExport: 3, storageAgentTransport: 2 }, observedAt: new Date('2026-08-30T00:00:00.000Z'),
      }] };
      return { rows: [] };
    },
  ...
  const runtimeReceipt = await runtime.reportRuntime({ agentId: 'storage-agent-1', token: 'storage-agent-runtime-test-token-with-sufficient-length', agentVersion: '8.8.0', contracts: { questionPaperExport: 3, storageAgentTransport: 2 } });
  assert.strictEqual(runtimeReceipt.agentVersion, '8.8.0');
  assert.strictEqual(
