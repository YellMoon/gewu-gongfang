...
const service = createMiniappCloudAccountService({
  ...
  canonicalWechatIdentity: {
    async resolveOrBind({ loginCode, phoneCode }) {
      ...
      assert.ok(['admin-proof', 'new-proof', 'guardian-proof', 'known-proof'].includes(phoneCode));
...
(async () => {
  ...
  const knownWechat = await service.login({ loginCode: 'known-login', phoneCode: 'known-proof' });
  assert.strictEqual(knownWechat.identity.accountId, admin.identity.accountId);
  ...
  await assert.rejects(() => service.login({ loginCode: 'known-login', phoneCode: null }), error => error.code === 'CLOUD_MINIAPP_IDENTITY_REJECTED');
