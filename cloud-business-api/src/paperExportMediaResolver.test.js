...
(async () => {
  ...
  const resolver = createPaperExportMediaResolver({
    questionAssetDeliveries: {
      requestForPaperExport: async input => { calls.push(['request', input]); return { deliveryId: 'question_asset_delivery_12345678', status: 'ready' }; },
  ...
  const input = { tenantId: 'default', accountId: 'account-1', taskId: 'paper_task_1', questionId: 'question-1', assetKey: 'a'.repeat(64), fileName: 'diagram.png', mimeType: 'image/png' };
  assert.deepStrictEqual(await resolver(input), bytes);
  assert.deepStrictEqual(calls, [
    ['request', { tenantId: 'default', accountId: 'account-1', taskId: 'paper_task_1', questionId: 'question-1', assetKey: 'a'.repeat(64) }],
  ...
  const pending = createPaperExportMediaResolver({
    questionAssetDeliveries: { requestForPaperExport: async () => ({ deliveryId: 'question_asset_delivery_12345678', status: 'queued' }), download: async () => { throw new Error('unexpected'); } },
  });
