...
(async () => {
  ...
  const pairingUnavailable = await request(createCloudBusinessApp({
    query: async () => ({ rows: [] }),
    desktopRegistration: identity,
    desktopPairing: {
  ...
  assert.strictEqual(pairingUnavailable.status, 503);
  assert.deepStrictEqual(pairingUnavailable.body, { ok: false, code: 'CLOUD_DESKTOP_PAIRING_UNAVAILABLE' });
