  ...
  return async function updateStudent(input) {
    const result = await query(
      `SELECT id AS "id", to_char(updated_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS "updatedAt"
       FROM business.vnext_update_student($1,$2,$3::timestamptz,$4,$5,$6,$7,$8,$9,$10)`,
      [input.tenantId, input.studentId, input.expectedUpdatedAt, input.name, input.school, input.gradeYear, input.gradeCurrent, input.institutionId, input.parentName, input.notes],
    );
