...
(async () => {
  ...
  await repository.submit({
    accountId: 'account-visitor', applicationId: 'role_application_0002', idempotencyKey: 'role-application-new-teacher-1',
    requestedIdentity: 'teacher', profileMode: 'new', bindingHint: 'New teacher profile', submittedAt: '2026-08-26T08:01:00.000Z',
  });
  assert.strictEqual(calls[2][1][5], 'new');
  const pending = await repository.listSubmitted();
  assert.strictEqual(pending.length, 1);
  assert.ok(calls[3][0].includes('business.vnext_list_submitted_cloud_role_applications_v2'));
  assert.deepStrictEqual(calls[3][1], ['default']);
  ...
  assert.ok(calls[4][0].includes('business.vnext_review_cloud_role_application_v2'));
  assert.deepStrictEqual(calls[4][1], ['default', 'account-super-admin', 'role_application_0001', 'approved', 'teacher-1', '2026-08-26T08:00:00.000Z']);
