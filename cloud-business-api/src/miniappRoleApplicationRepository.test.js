...
(async () => {
  ...
  assert.ok(calls[0][0].includes('business.cloud_role_applications') && calls[0][0].includes('ORDER BY submitted_at DESC,application_id DESC LIMIT 1'));
  ...
  assert.ok(calls[1][0].includes('INSERT INTO business.cloud_role_applications') && calls[1][0].includes('ON CONFLICT (tenant_id,cloud_account_id,idempotency_key) DO UPDATE'));
  ...
  assert.ok(calls[2][0].includes("status='submitted'"));
  ...
  assert.ok(calls[3][0].includes('business.vnext_review_cloud_role_application_v1'));
