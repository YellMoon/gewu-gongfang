...
One user-facing release has one `Major.Minor.Patch` version. Desktop, the local data host, Backend, Gateway, and the WeChat miniapp development build must all use that exact version. If any required endpoint has no same-version receipt, the release is **partial**, never complete.
...
| `local_host` | Actual local data-host runtime and question-drive checks at the release version |
...
6. Publish OSS with `npm run publish:desktop-update`; the successful feed and installer upload records the desktop receipt.
7. Upgrade and inspect the local data host without touching real business data, then record its receipt:

   ```powershell
   node scripts/release-matrix.js record --target local_host --version <release-version> --evidence "local runtime and question-drive checks passed"
   ```

8. Run `npm run miniapp:upload`. The upload refuses a manual `--version` that differs from the ledger and writes the miniapp receipt only after success.
9. Use `npm run release:status`, then `npm run release:complete`. Completion requires all five exact-version receipts.
...
- `npm run publish:desktop-rollback -- --rollback=<archived-version>` changes only the OSS desktop feed. It does not claim that cloud services, data hosts, or miniapp builds were rolled back.
...
Post-release evidence must include the live OSS feed, local host runtime, Backend health, Gateway health, and WeChat upload receipt. Git state or a local build is not substitute evidence.
