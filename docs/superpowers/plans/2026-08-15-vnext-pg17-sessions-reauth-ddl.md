...
- Add four session guards (insert/current-parent, identity-immutable, monotonic lifecycle, no-delete) and three reauthentication guards (online/window/current-parent, no-update, no-delete). Every function is owner-owned SECURITY DEFINER with `pg_catalog,pg_temp`; verifier receives SELECT only; runtime receives no rights.
...
- [x] Import `SESSIONS_REAUTHENTICATION_MIGRATION`; require `[1..15]`, both relations, and all session/reauth trigger names.
- [x] Assert exact columns, four session parent FKs, reauth session FK, enum/time/lifecycle clauses, and seven function/trigger names.
- [x] Run `node shared/vnext-pg17/migrationManifest.test.js`; observed the intended red failure before the M15 catalog existed.
- [x] Add only M15 SQL, manifest record, function hashes, relation/trigger manifests, and export; M1-M14 SQL is unchanged.
- [x] Rerun the manifest test; `vNext PG17 migration manifest checks passed`.
...
- [x] Hand-apply M1-M14: require catalog apply/assert failure, exact `[1..14]` ledger, and both M15 relations/functions absent.
- [x] Use only synthetic active authority/account/device/installation/link parents. Prove zero seed, valid online/initialization sessions, and valid password/passkey/verified-contact reauth events.
...
- [x] Freeze both column sets, owners, exact constraints/indexes/FKs, ACLs, function security/path/source hashes, and full trigger facts.
...
- [x] Record sanitized synthetic evidence only; focused tests, aggregate gate, `git diff --check`, and the label query passed with no labelled container left behind.
