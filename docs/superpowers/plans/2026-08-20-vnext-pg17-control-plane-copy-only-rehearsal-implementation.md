...
**Current implementation status (2026-08-20):** The delivered scope is `link-revocation evidence boundary-verified`: it retains the synthetic disposable identity lifecycle, historical authorization, and opaque profile-binding boundaries, and additionally imports only structurally validated historical `account_device_link.revoke` receipts, one audit event per receipt, and accepted-only outbox events. Canonical request/result/context/payload bytes and hashes, target versions, source-level uniqueness, reread hashes, and transaction rollback are checked; this is historical consistency only, never present authorization or command replay. Link parents retain the existing FK-only structural rule; session/resolver admission remains a later, separate boundary. Active profile bindings require active accounts and remain non-authorizing. Contacts, all other receipts/audit/outbox records, sessions, reauthentication, policy, trust evidence, and every legacy collection remain non-writing boundaries. This is not a complete rehearsal, real-data migration, or release evidence.
