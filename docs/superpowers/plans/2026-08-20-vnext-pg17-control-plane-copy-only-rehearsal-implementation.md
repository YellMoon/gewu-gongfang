...
**Current implementation status (2026-08-20):** The delivered scope is `verified-contact metadata boundary-verified`: it retains the synthetic disposable identity lifecycle, historical authorization, opaque profile-binding, and link-revocation evidence boundaries, and additionally imports only structurally validated `verified` or `revoked` contact metadata. Contact values remain opaque, are never normalized, looked up, or used for login, recovery, sessions, reauthentication, AccessContext, or command authorization. A verified contact requires an active mapped account; a revoked contact may remain as history on a disabled or revoked account. Exact source shape, lifecycle, nonpartial identity uniqueness, static target SQL, reread hashes, rollback, and target poisoning are checked. All other receipts/audit/outbox records, sessions, reauthentication, policy, trust evidence, and every legacy collection remain non-writing boundaries. This is not a complete rehearsal, real-data migration, or release evidence.
