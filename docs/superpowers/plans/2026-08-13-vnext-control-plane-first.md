...
- Synthetic PG17 checks cover desktop capability derivation, miniapp surface filtering, absent reauthentication, session-expiry equality, forged assertions, account access-vector drift, and a peer write between two reads of a stable snapshot. This remains an internal disposable reference only: no token or credential verifier, API, runtime integration, RDS/ECS access, desktop/business data, NAS, removable media, or deployment was used.
