...
### PostgreSQL 17 disposable target-ledger evidence (2026-08-15; initial harness slice)

- This initial target-engine harness slice added exact `pg` `8.23.0`, a digest-pinned official PostgreSQL 17 image, closure-private handles, and a fixed `vnext_control_plane` schema inside one local Docker container bound to `127.0.0.1`. It accepts no caller connection configuration and does not contact RDS, ECS, a cloud database, NAS, removable media, or source/business data.
- At this initial checkpoint the sole approved target relation was the append-only `vnext_control_plane.vnext_schema_migrations` ledger. It is owned by a synthetic non-login owner, has ordered-version and no-update/no-delete guards, and is checked through a read-only verifier identity. The later M2–M15 evidence below expands the local synthetic target; this paragraph is not a statement of the current relation set.
...
- Fresh focused checks, `npm run test:vnext-control-plane-target`, a repeated disposable-runtime run with no labelled container left behind, `npm test`, and `git diff --check` passed locally at that checkpoint. This was disposable synthetic validation only; non-production RDS validation, real migrations, APIs, and deployment remained deferred. Current delivery status is tracked in `docs/superpowers/inventories/2026-08-21-vnext-cloud-authority-delivery-readiness.md`.
...
- The policy-read matrix additionally proves active-role defaults, revoked/expired-role visitor fallback, half-open override and scope windows, capability deny precedence, and no capability-catalog read as a second policy truth. Scope values remain opaque nonblank text (including colon-bearing values): pure policy locks same-key allow/deny precedence, while the resolver accepts only catalog-valid distinct active tuples and preserves their allow/deny result. Each matrix case asserts verifier-only read trace and unchanged logical rows across all M1-M15 relations.
- The recent-reauthentication selection matrix independently reads the existing password, passkey, and verified_contact event-factor values, returns only the greatest eligible `expires_at`, accepts `verified_at = now`, excludes `expires_at = now`, and never leaks factor/evidence fields. It does not query or treat `vnext_verified_contacts` metadata as a credential. Standalone resolver verification reports success only after its disposable container has stopped and its ownership-label set returns to its initial baseline.
