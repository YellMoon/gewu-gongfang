...
**Architecture:** A new closure-branded parity boundary accepts the same-handle opaque session assertion and an exact five-field command. It reuses the existing AccessContext resolver's verifier-only read boundary, then derives canonical request/result/audit/outbox vectors in its own repeatable-read, read-only verifier transaction. Its test compares those values with the existing synthetic mutation's durable output; it grants no privilege and imports no write facade.
...
- [x] **Step 1: Build a synthetic fixture with an active actor, target link, policy, session and reauthentication event**
...
- [x] **Step 2: Write the first red assertion for an accepted vector**
...
- [x] **Step 1: Add exact configuration and command snapshots**
...
- [x] **Step 2: Bind an opaque resolver and canonical clock**
...
- [x] **Step 3: Derive vectors in a verifier-only read transaction**
...
- [x] **Step 4: Run green accepted-vector test**
...
- [x] **Step 1: Add golden vectors for replay, conflict and non-accepted outcomes**
...
- [x] **Step 2: Add authorization and input fail-closed tests**
...
- [x] **Step 3: Add read-only trace tests**
...
- [x] **Step 4: Register the focused parity suite**
...
- [x] **Step 1: Record the admission gate**
...
- [x] **Step 2: Run independent audit**
