...
- [x] 在原 Task 13 完成 6.4.0 统一发布和当前主机 generation 1 bootstrap；不在未获用户实际换机要求时切换主机。
- [ ] 在第二台真实电脑完成普通端注册、当前主机自动批准、双角色/teacher_id、同步和撤销验收。
...
2026-07-23：后续 6.4.0 主机包已在备份后安装并保留原数据目录与设备配置；本地健康为 6.4.0，配置为 `primary-host/single-user`，active host epoch 为 generation 1。本机备份位于 `D:\GewuDataHost\backups\release-6.4.0-20260723-135335`，权威库与备份库 quick_check 均为 `ok`。生产 active desktop authorization 仍只有 1 条，因此第二台普通桌面端验收不包含在本步骤完成结论中。
...
当前结论为“部分发布”：6.4.0 阿里云、两条 OSS feed、本机主机安装与 generation 1 bootstrap、小程序开发版本上传和夸克交付均完成；微信正式审核/发布受主体与平台流程限制，第二台真实电脑的注册批准、双角色、同步、撤销与计划换机 dry-run 仍未完成。
