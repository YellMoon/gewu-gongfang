...
**Status:** source structure verified; target admission remains fail-closed. `subjects` remains a canonical but **unmapped** legacy-source candidate. This document does not authorize target DDL, a source-field mapping, a migration-admission batch, a shadow write, or a cloud write.
...
The exact historical `subjects` SQLite DDL and its SHA-256 are now recorded in [the redacted source-structure evidence](../../verification-2026-08-21-vnext-subjects-source-structure.md). The source has zero `subjects` rows and matches the recorded whole-source inventory baseline. The checked-in bootstrap schema therefore remains only a corroborating development artifact, not a substitute for real source evidence.
...
- reading additional source relations, exporting source rows, creating another snapshot, or scanning beyond the verified relation without its own read-only admission step;
...
1. A reviewed field mapping with exact source-to-target values, a stable-ID rule, a transformer identifier, dependency order, invariants, file-reference declaration, and rollback/reconciliation proof. A name-derived ID, generated replacement ID, or implicit default tenant is prohibited.
2. A tenancy decision proving how each nonblank source `tenant_id` maps to an already admitted `business.tenants(id)` value. Missing, ambiguous, blank, or cross-tenant references must be quarantined; they must never be silently repaired.
3. A canonical parsing contract for `deleted`, `created_at`, and `updated_at`: boolean representation, finite UTC timestamps, and `updated_at >= created_at`. The zero-row source cannot establish these semantics; unknown or ambiguous encodings must be quarantined, not normalized by guesswork.
4. A data-classification decision for `name` and `grade_level`, including log, audit, export, API, and tenant-scoped read rules. Neither field is currently approved as a credential, identifier for matching a person, or cross-tenant lookup key.
5. A proof that subject semantics do not need a parent curriculum, school, grade-system, locale, or version relation before they can be represented safely. If any such parent is required, this single-table admission is no-go and the dependency must be designed first.

Only after these five items can a separate target-DDL admission propose types, constraints, indexes, ownership, and column-level read permissions. Only after that later DDL has its own disposable catalog/ACL/zero-seed proof may a separate batch/row-ledger/reconciliation design consider synthetic rows.
...
After saving the verified structure evidence, the correct state is **WAIT** for a reviewed field mapping plus tenant-resolution, value-parsing/data-classification, and parent-semantics decisions. The next operation must not be a speculative DDL implementation or a broader mapping pass merely to create momentum.
