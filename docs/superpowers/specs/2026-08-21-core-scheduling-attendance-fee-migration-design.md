...
The reported 3013 device-local changes are not present in the approved SQLite snapshot: its sync log, conflict, and rejection tables are empty. They are therefore not imported as raw records. A later, separately authorized local-draft intake must read a specific device-local root, verify its command shape and snapshot identity, show an impact preview, and submit each schedule create/change/delete, attendance change, or fee change as an idempotent user-confirmed cloud command.
