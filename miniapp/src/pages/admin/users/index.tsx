...
export default function AdminUsersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!canRead) {
        setError('\u5f53\u524d\u8d26\u53f7\u65e0\u6743\u67e5\u770b\u7528\u6237\u5206\u7c7b');
        setUsers([]);
        return;
      }
      const isReviewer = nextCapabilities.includes('users:review');
      const requests: Promise<any>[] = [adminApi.getUsers({ page: 1, search: submittedSearch, review_status: status })];
  ...
  const reviewUser = async (user: any, role: typeof roles[number]) => {
    if (!canReview || savingUserId) return;
  ...
  const disableUser = async (user: any) => {
    if (!canReview || savingUserId || user.phone === SUPER_ADMIN_PHONE) return;
  ...
  return <View className="admin-page">
    ...
    {canReview && pairings.length > 0 ? <View className="pairing-section"><Text className="section-title">{'\u5f85\u5ba1\u6838\u8bbe\u5907'}</Text>{pairings.map(item => <View className="pairing-item" key={item.id || item.pairingCode}><Text>{item.deviceName || '\u672a\u547d\u540d\u8bbe\u5907'} {item.phone || ''}</Text><View><Button size="mini" onClick={() => reviewPairing(item.pairingCode, 'approve')}>{'\u901a\u8fc7'}</Button><Button size="mini" onClick={() => reviewPairing(item.pairingCode, 'reject')}>{'\u62d2\u7edd'}</Button></View></View>)}</View> : null}
    <View className="user-list">
      ...
      {!loading && !error ? users.map(user => {
        ...
        return <View key={user.id} className={`user-item ${disabled ? 'disabled' : ''}`}>
          ...
          {canReview && !isFixedSuperAdmin ? <View className="user-actions"><Picker disabled={savingUserId === user.id} mode="selector" range={roles.map(role => roleLabels[role])} value={Math.max(0, roles.indexOf(userRole as typeof roles[number]))} onChange={event => void reviewUser(user, roles[Number(event.detail.value)])}><View className="type-selector">{savingUserId === user.id ? '\u4fdd\u5b58\u4e2d\u2026' : '\u5ba1\u6838\u5206\u7c7b'}</View></Picker>{!disabled ? <Button className="disable-btn" size="mini" disabled={Boolean(savingUserId)} onClick={() => void disableUser(user)}>{'\u505c\u7528'}</Button> : null}</View> : null}
        </View>;
      }) : null}
    </View>
  </View>;
