...
assert.match(source, /await fetchPermissions\(\)/, 'home must refresh the authenticated cloud authorization before loading role-scoped data');
assert.ok(!source.includes('/api/permissions'), 'home must not call a retired local-backend permission endpoint');
