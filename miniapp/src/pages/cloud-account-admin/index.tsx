...
export default function CloudAccountAdminPage() {
  ...
  const assign = async (accountId: string) => {
    ...
    const response = await miniappCloudBusinessApi.assignAccountRole(token, accountId, role);
    if (response.success) {
  ...
  return <View className='cloud-account-admin-page'>
    ...
    {accounts.map(account => {
      ...
      return <View className='cloud-account-admin-card' key={account.accountId}>
        ...
        <Picker mode='selector' range={ROLE_OPTIONS.map(option => option.label)} value={roleIndex} onChange={event => setRoleIndexByAccount(current => ({ ...current, [account.accountId]: Number(event.detail.value) }))}>
