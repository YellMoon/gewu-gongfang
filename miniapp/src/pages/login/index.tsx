    ...
    return <View className="login-page">
      ...
      <View className="desktop-login-card">
        {desktopLoginState === 'success' ? <>
          ...
          {hasDesktopSession
            ? <Button className="wx-login-btn" onClick={() => void handleDesktopSessionLogin()} loading={loading} disabled={loading}>{'\u786e\u8ba4\u767b\u5f55'}</Button>
            : <Button className="wx-login-btn" openType="getPhoneNumber" onGetPhoneNumber={(event) => void handleDesktopPhoneLogin(event?.detail?.code || '')} loading={loading} disabled={loading}>{'\u624b\u673a\u53f7\u5feb\u6377\u767b\u5f55'}</Button>}
