...
export default function QuestionBankPage() {
  ...
  return <View className='question-bank-page'>
    ...
    <View className='action-card'>{(['question-paper', 'paper-export-word', 'paper-export-pdf'] as PaperAction[]).map(action => <Button key={action} className={`action-button ${action}`} loading={submitting === action} disabled={Boolean(submitting) || previewState !== 'ready' || selectedIds.length === 0} onClick={() => submit(action)}>{actionCopy[action]}</Button>)}</View>
