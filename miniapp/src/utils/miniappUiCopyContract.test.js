...
const forbiddenPage = display(read('miniapp/src/components/ForbiddenContent.tsx'));
assert.match(read('miniapp/src/pages/forbidden/index.tsx'), /return <ForbiddenContent\s*\/>/, 'registered forbidden route renders the shared access explanation');
...
const operationsShortcutTitle = String.fromCharCode(36816, 33829, 24555, 25463, 20837, 21475);
const teachingToolsTitle = String.fromCharCode(25945, 23398, 24037, 20855);
const readOnlyPaperTitle = String.fromCharCode(21482, 35835, 19982, 32452, 21367);
const commonContentTitle = String.fromCharCode(24120, 29992, 20869, 23481);
const commonManagementTitle = String.fromCharCode(24120, 29992, 31649, 29702);
const commonRecordsTitle = String.fromCharCode(24120, 29992, 36164, 26009);
assert.ok(!homePage.includes(operationsShortcutTitle) && homePage.includes(teachingToolsTitle), 'the teacher home must not frame teaching shortcuts as operations');
assert.ok(!homePage.includes(readOnlyPaperTitle) && homePage.includes(commonContentTitle), 'the student home must not explain a restriction before the user attempts a restricted action');
assert.ok(!homePage.includes(commonManagementTitle) && homePage.includes(commonRecordsTitle), 'the teacher shortcut area must name the records it opens rather than an internal management role');
