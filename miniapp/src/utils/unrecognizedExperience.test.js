...
assert.ok(page.includes('isUnrecognizedIdentity'), 'experience page must verify the shared authoritative identity');
assert.ok(page.includes('openSessionBoundDocument'), 'downloaded artifacts must stay bound to the current auth session');
assert.ok(page.includes("'/pages/account-application/index'"), 'experience entry must open the single account application page');
