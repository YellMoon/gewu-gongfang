...
function normalizedManualPhone(value) {
  const phone = String(value || '').trim().replace(/^\+?86/, '').replace(/\D/g, '');
  if (!/^1[3-9]\d{9}$/.test(phone)) throw runtimeError('MANUAL_PHONE_INVALID');
  return phone;
}
...
function buildDesktopConfirmationPayload({ challengeId, loginCode, phone, expectedRowVersion } = {}) {
  return Object.freeze({
    challengeId: normalizedChallengeId(challengeId),
    code: normalizedWechatCode(loginCode, 'WECHAT_LOGIN_CODE_REQUIRED'),
    phone: normalizedManualPhone(phone),
