...
## 0. 当前执行目标：桌面端一键同步（2026-07-09）

### 用户确认后的产品规则

- 主机电脑和其他电脑都由同一位用户本人使用，不需要做多人审批流。
- 其他电脑只要能联网，就能在软件内点击一个主按钮完成同步。
- 同步前必须在当前电脑展示变动摘要，并由用户确认；取消后不上传、不拉取、不改本地数据。
- 不在同一个 Wi-Fi 时必须支持互联网同步，走阿里云中继。
- 在同一个局域网时优先直连本地数据主机；直连失败再走阿里云中继。
- 主机电脑未开机时，其他电脑允许把同步请求提交到阿里云中继，状态显示为等待主机上线处理。
- 阿里云仍只做中继、任务队列和状态缓存，不成为最终权威业务数据库。
- 权威业务数据库仍在本地数据主机，题库移动硬盘仍由本地数据主机统一管理。

### 一键同步验收标准

- 桌面端云同步页面有明确的“一键同步”主按钮。
- 点击后自动选择连接通道：局域网直连优先，失败后使用云中继。
- 执行前展示变动预览：将上传、本机删除、将拉取、冲突/高风险项。
- 用户确认后才执行同步；用户取消时本地待同步队列保持不变。
- 有本机离线变更时，先上传到主机或云中继；没有本机变更时也能从主机拉取最新数据。
- 主机离线且走云中继时，返回“已提交，等待主机上线处理”，不误报同步完成。
- 同步完成后展示上传条数、拉取条数、冲突条数和所用通道。
- 同步失败时保留本机 pending queue，并给出可理解的错误提示。
- 相关行为必须有自动化测试覆盖，至少覆盖：直连优先、直连失败走云、用户取消、确认后执行、主机离线等待、失败保留队列。

### 当前实施清单

- [ ] 设计并测试桌面端一键同步核心服务。
- [ ] 增加同步变动预览汇总逻辑。
- [ ] 将 `SyncSettings` 页面收敛为“一键同步”主流程。
- [ ] 补齐云中继同步任务 API 与主机处理流程。
- [ ] 增加主机后台轮询云端待处理同步任务。
- [ ] 完成全量测试、打包、提交、推送和 OSS 更新包发布。
