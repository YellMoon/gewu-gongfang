...
Source 9b5d87bf is pushed. Final-version build and 183/183 frozen-source cloud
test commands passed; deployment tests 56 passed. Cloud receipt verified
2026-09-23T12:02:30Z at gewu-cloud-81121-release-20260923-xb39l0jf/active.json;
fresh backup/restore/security checks passed before promotion. Guarded miniapp
development upload verified 2026-09-23T12:03:51.078Z in the SAME manifest;
receipt gewu-miniapp-8815-upload-20260923-g613xt28. Desktop/NAS unchanged.
Next: reversible real import/replay/owner readback and populated layout check,
then remaining page-detail audit. Controlled import responses are not real
financial writes. Full project and formal miniapp release remain unproven.
...
> - [ ] Desktop: use an isolated packaged-app profile to prove online login/device registration, offline draft persistence, no silent publish after reconnect, explicit confirmation, cloud submission, projection refresh/readback, and conflict presentation without touching the user's normal profile.
> - [ ] Miniapp: use the uploaded development build against the production API to exercise real admin and student/parent read scopes, offline/forbidden states, and the allowed personal-asset/question-paper/export task paths. Do not mutate core teaching or question-bank base data.
> - [ ] Re-run cloud/NAS/desktop/miniapp release evidence after acceptance, then update the unified release matrix. Formal WeChat production review remains a separate human gate.
>
> **Rollback/preservation:** every production mutation uses a unique `codex-e2e-8.4.1-*` marker, is limited to a disposable record or task, and must be deleted/cancelled in `finally`. Existing business rows, the retained legacy source, user profiles, protected `output/` directories, NAS objects, and AList state are not modified by the acceptance harness.
...
>
> The approved legacy desktop root is read-only inventory input. The user has confirmed that the question bank has no migration data and that assets are empty. The 80 `questions`, 80 `question_contents`, and 911 `question_assets` rows discovered structurally are therefore legacy import/index residue, not approved question-bank content: they are excluded from migration without further content investigation and remain only in the retained legacy snapshot. Migration starts with the small, user-confirmed core-business allow-list; no missing domain may be invented, searched for in another private location, or declared migrated without evidence.
>
...
>
> The checked-in 99-table source catalog is complete as a structural intake gate. The tenant/institution/school/room foundation has four reviewed proposed logical field contracts, verified against the redacted physical SQLite DDL/column shape, independently ledgered empty local-disposable PostgreSQL 17 DDL proof, and a closure-owned fictional four-relation shadow-admission proof: immutable batch/row provenance, exact replay, read-only target reconciliation, terminal poison, and whole-disposable-target destruction. The approved legacy source has been read only for the recorded 99-table inventory/snapshot baseline, the four-table structural contract, and the redacted zero-row `subjects` structural contract; no export, RDS/NAS write, real shadow batch, restore, cutover, or release has occurred. Fourteen remaining core-business candidates are deliberately `unmapped`, and it is impossible to advance any of them by changing a flag alone. Question-labelled and legacy credential relations are fail-closed; real shadow admission remains blocked until each admitted table has its exact reviewed logical contract, target DDL, source snapshot, reconciliation, restore, and rollback evidence.
>
