...
- [x] 2026-07-22 完成未认可学生生产安全探针：允许范围 3/3 为 200，正式 Backend 范围 9/9 为 403，Gateway 2/2 为 401，旧审核入口 2/2 为 410；PM2 日志增量和登录事件表结构未发现临时令牌、Authorization 头或微信临时凭证落盘。小程序上传与审核版本核验仍归上一条待办。
...
- [ ] 维护并验证 `miniapp/src/app.config.ts` 全部 17 个注册页面；`miniappUiCoverage` 已通过，仍需分别留下未认可学生、受认可学生、家长、老师、普通管理员、超级管理员及空态/离线/无权限/有限写入关键状态的当前版本真实微信截图。
