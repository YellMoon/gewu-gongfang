...
function canonical(value) { return [value.taskId, value.actorUserId, value.deviceId, value.issuedAt, value.nonce].join('\n'); }
...
function issueRelayAssertion(claims, secret) {
  ...
  const payload = { taskId:claims.taskId, actorUserId:claims.actorUserId, deviceId:claims.deviceId,
    issuedAt:Number(claims.issuedAt || Date.now()), nonce:claims.nonce || crypto.randomBytes(18).toString('hex') };
...
function verifyRelayAssertion(assertion, secret, options = {}) {
  ...
  return { taskId:assertion.taskId, actorUserId:assertion.actorUserId, deviceId:assertion.deviceId,
