...
function createAuthorityRoleMirrorService({ db } = {}) {
  ...
  const replace = db.transaction(projection => {
    ...
    const signature = text(
      ...
      'AUTHORITY_ROLE_MIRROR_SIGNATURE_REQUIRED',
      2048,
    );
    const payloadHash = text(
      projection.payloadHash,
      'AUTHORITY_ROLE_MIRROR_PAYLOAD_HASH_REQUIRED',
      128,
    );
    const generatedAt = text(
      projection.generatedAt,
      'AUTHORITY_ROLE_MIRROR_TIMESTAMP_REQUIRED',
      64,
    );
    ...
    const grants = list(
      projection.payload?.roleGrants,
      'AUTHORITY_ROLE_GRANT_MIRRORS_INVALID',
    );
    ...
    for (const grant of grants) {
      ...
      insertAuthorizationGrant.run(
        grant.bindingId,
        authorityId,
        grant.userId,
        grant.role,
        grant.subjectType || null,
        grant.subjectId || null,
        grant.status,
        Number(grant.grantVersion),
        grant.grantedBy || null,
        grant.createdAt || generatedAt,
        grant.updatedAt || generatedAt,
  ...
  return Object.freeze({
    replaceFromVerifiedProjection: projection => replace(projection),
  });
}
