...
(async () => {
  ...
  assert.strictEqual((await call('/desktop-sync/requests',{method:'POST',headers:{'x-test-user':teacher('teacher1'),'x-device-id':'cloud-d1'},body:'{}'})).status,200,'approved pairing remains durable after request-window expiry');
  getDb().prepare("UPDATE desktop_device_pairings SET status='pending' WHERE id='pair-cloud-d1'").run();
  assert.strictEqual((await call('/desktop-sync/requests',{method:'POST',headers:{'x-test-user':teacher('teacher1'),'x-device-id':'cloud-d1'},body:'{}'})).status,403);
