...
router.post('/desktop-sync/requests',requireApprovedSnapshotUser,(req,res)=>{const invalid=validateDesktopSyncInput(req,res);if(invalid)return invalid;const db=getDb(),deviceId=req.headers['x-device-id']||req.body.deviceId,device=db.prepare('SELECT * FROM cloud_devices WHERE id=? AND active=1').get(deviceId);if(!device||device.owner_user_id!==req.user.id)return res.status(403).json({success:false,code:'SYNC_DEVICE_OWNER_MISMATCH'});const pairing=db.prepare("SELECT id FROM desktop_device_pairings WHERE device_id=? AND user_id=? AND status='approved' ORDER BY updated_at DESC LIMIT 1").get(deviceId,req.user.id);if(!pairing)return res.status(403).json({success:false,code:'PAIRING_NOT_APPROVED'});const taskId=id('desktop_sync'),time=now();let assertion;try{assertion=issueRelayAssertion({taskId,actorUserId:req.user.id,deviceId,pairingApprovalId:pairing.id},process.env.GEWU_CLOUD_RELAY_HOST_TOKEN||'');}catch(e){return res.status(403).json({success:false,code:e.code});}const payload={deviceId,tenantId:req.body.tenantId||'default',pendingChanges:req.body.pendingChanges||[],actorUserId:req.user.id,relayAssertion:assertion,submittedAt:time};db.prepare("INSERT INTO miniapp_tasks(id,task_type,status,payload,created_by,created_at,updated_at)VALUES(?,'desktop-sync','pending_host',?,?,?,?)").run(taskId,JSON.stringify(payload),req.user.id,time,time);res.json({success:true,request:{id:taskId,status:'pending_host',acceptedChanges:payload.pendingChanges.length}});});
router.get('/desktop-sync/requests/:id/result',requireApprovedSnapshotUser,(req,res)=>{const db=getDb(),admin=['super_admin','admin'].includes(roleForUser(req.user));const row=admin?db.prepare("SELECT * FROM miniapp_tasks WHERE id=? AND task_type='desktop-sync'").get(req.params.id):db.prepare("SELECT * FROM miniapp_tasks WHERE id=? AND task_type='desktop-sync' AND CAST(created_by AS TEXT)=?").get(req.params.id,String(req.user.id));if(!row)return res.status(404).json({success:false,code:'DESKTOP_SYNC_REQUEST_NOT_FOUND'});res.json({success:true,request:{...row,payload:JSON.parse(row.payload||'{}'),result_payload:row.result_payload?JSON.parse(row.result_payload):null}});});
