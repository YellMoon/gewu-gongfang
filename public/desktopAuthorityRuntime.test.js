...
(async function main() {
  ...
  const calls = [];
  let offlineLeaseStatus = Object.freeze({
    state: 'unlocked',
    unlocked: true,
  ...
  const runtime = createDesktopAuthorityRuntime({
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (url === 'https://business.example/api/desktop/question-bank/commands') {
      ...
      if (url === 'https://business.example/api/desktop/role-applications/role-runtime-1/review') {
        assert.strictEqual(options.method, 'POST');
        assert.strictEqual(options.headers.authorization, 'Bearer desktop-session-token');
        assert.deepStrictEqual(JSON.parse(options.body), { decision: 'approved', profileId: null });
  ...
  assert.strictEqual(await runtime.submit(draft.id, { sessionToken: 'desktop-session-token' }), undefined);
  ...
  assert.ok(!calls.some(call => call.url === 'https://control.example/api/desktop/question-bank/commands'),
    'question drafts must use the cloud business authority, never the retired scheduling relay');
  ...
  const reviewedRoleApplication = await runtime.reviewRoleApplication('role-runtime-1', {
    decision: 'approved', profileId: null,
  ...
  const firstMergedUpdate = runtime.appendDraftSync({
    type: 'student.update.v1',
    payload: {
      id: 'student-merge-update-1',
      expectedVersion: '2026-07-27T01:00:00.000Z',
      changes: { name: 'First local name', notes: 'first local note' },
    },
  });
  const secondMergedUpdate = runtime.appendDraftSync({
    type: 'student.update.v1',
    payload: {
      id: 'student-merge-update-1',
      expectedVersion: '2026-07-28T01:00:00.000Z',
      changes: { name: 'Latest local name' },
    },
  });
  assert.strictEqual(secondMergedUpdate.id, firstMergedUpdate.id,
    'successive offline edits must reuse one pending draft');
  assert.deepStrictEqual(secondMergedUpdate.payload, {
    id: 'student-merge-update-1',
    expectedVersion: '2026-07-27T01:00:00.000Z',
    changes: { name: 'Latest local name', notes: 'first local note' },
  });
  const createdThenUpdated = runtime.appendDraftSync({
    type: 'room.create.v1',
    payload: { record: { id: 'room-merge-create-1', name: 'Draft room', address: null } },
  });
  const mergedCreate = runtime.appendDraftSync({
    type: 'room.update.v1',
    payload: {
      id: 'room-merge-create-1',
      expectedVersion: 'local-version-must-not-survive',
      changes: { name: 'Latest draft room' },
    },
  });
  assert.strictEqual(mergedCreate.id, createdThenUpdated.id,
    'create then update must remain one create draft');
  assert.strictEqual(mergedCreate.type, 'room.create.v1');
  ...
  const canceledResult = runtime.appendDraftSync({
    type: 'school.delete.v1',
    payload: { id: 'school-merge-cancel-1', expectedVersion: 'local-version-must-not-survive' },
  });
  assert.strictEqual(canceledResult, undefined, 'create then delete must cancel the pending draft');
  await assert.rejects(
    () => runtime.get(canceledCreate.id),
    error => error?.code === 'AUTHORITY_OUTBOX_ITEM_NOT_FOUND',
  );
  const updatedThenDeleted = runtime.appendDraftSync({
    type: 'course.update.v1',
    payload: {
      id: 'course-merge-delete-1',
      expectedVersion: '2026-07-27T02:00:00.000Z',
      changes: { name: 'Pending course update' },
    },
  });
  const mergedDelete = runtime.appendDraftSync({
    type: 'course.delete.v1',
    payload: { id: 'course-merge-delete-1', expectedVersion: 'local-version-must-not-survive' },
  });
  assert.strictEqual(mergedDelete.id, updatedThenDeleted.id,
    'update then delete must reuse one pending draft');
  assert.strictEqual(mergedDelete.type, 'course.delete.v1');
  assert.deepStrictEqual(mergedDelete.payload, {
    id: 'course-merge-delete-1',
    expectedVersion: '2026-07-27T02:00:00.000Z',
  });
  const firstAsyncUpdate = await runtime.appendDraft({
    ...
    payload: {
      id: 'payment-merge-async-1',
      expectedVersion: '2026-07-27T03:00:00.000Z',
      changes: { amount: 100 },
    },
  });
  const secondAsyncUpdate = await runtime.appendDraft({
    type: 'payment.update.v1',
    payload: {
      id: 'payment-merge-async-1',
      expectedVersion: 'local-version-must-not-survive',
  ...
  assert.strictEqual(secondAsyncUpdate.id, firstAsyncUpdate.id,
    'async draft capture must use the same net-change merge boundary');
  assert.strictEqual(secondAsyncUpdate.payload.expectedVersion, '2026-07-27T03:00:00.000Z');
  const malformedCreate = runtime.appendDraftSync({
    type: 'school.create.v1', payload: { record: { name: 'Missing id' } },
  });
  const malformedUpdate = runtime.appendDraftSync({
    type: 'school.update.v1', payload: { changes: { name: 'Still missing id' } },
  });
  assert.notStrictEqual(malformedUpdate.id, malformedCreate.id,
    'malformed drafts without a real string record id must never be merged');
  const firstAccountDraft = runtime.appendDraftSync({
    type: 'consumption.update.v1',
    payload: { id: 'shared-record-id', expectedVersion: '2026-07-27T04:00:00.000Z', changes: { amount: 10 } },
  });
  offlineLeaseStatus = Object.freeze({
    ...offlineLeaseStatus,
    user: Object.freeze({ id: 'user-2' }),
    authorizationId: 'authorization-2',
    offlineLease: Object.freeze({
      ...offlineLeaseStatus.offlineLease,
      userId: 'user-2',
      authorizationId: 'authorization-2',
    }),
  });
  const secondAccountDraft = runtime.appendDraftSync({
    type: 'consumption.update.v1',
    payload: { id: 'shared-record-id', expectedVersion: '2026-07-27T05:00:00.000Z', changes: { amount: 20 } },
  });
  assert.notStrictEqual(secondAccountDraft.id, firstAccountDraft.id,
    'net-change merging must never cross account authorization scopes');
  offlineLeaseStatus = Object.freeze({
    ...offlineLeaseStatus,
    user: Object.freeze({ id: 'user-1' }),
    authorizationId: 'authorization-1',
  ...
  for (const retired of ['/api/authority/commands', '/api/authority/projections/current', 'createAuthorityWebSocketTransport']) {
    assert.ok(!runtimeSource.includes(retired), `desktop runtime must not retain ${retired}`);
  }
  ...
  assert.ok(!electronSource.includes('desktop-authority:read-projection'));
  assert.ok(!preloadSource.includes('desktop-authority:read-projection'));
