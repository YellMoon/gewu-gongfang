...
class QuestionDraftProvenanceRegistry {
  ...
  async register(questionId, token) {
    ...
    const data = this._read(); const existing = data[questionId];
    if (existing && (existing.userId !== claims.userId || existing.deviceId !== claims.deviceId)) throw Object.assign(new Error('QUESTION_DRAFT_OWNER_IMMUTABLE'), { code: 'QUESTION_DRAFT_OWNER_IMMUTABLE' });
    if (!existing) { data[questionId] = { userId: claims.userId, deviceId: claims.deviceId, registeredAt: new Date().toISOString() }; this._write(data); }
